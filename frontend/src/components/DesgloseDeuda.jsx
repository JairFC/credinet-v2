  ...
  const fetchDebtData = async () => {
    try {
      ...
      const summaryResponse = await fetch(
        `${API_BASE_URL}/associates/${associateId}/debt-summary`,
        {
      ...
      if (!summaryResponse.ok || !summaryResult.success) {
        throw new Error(summaryResult.detail || 'Error al cargar resumen de deuda');
      }

      setDebtSummary(summaryResult.data);

      // Fetch all payments
      const paymentsResponse = await fetch(
        `${API_BASE_URL}/associates/${associateId}/all-payments`,
        {
          headers: { 'Authorization': `Bearer ${token}` }
        }
      );
      ...
      if (paymentsResponse.ok && paymentsResult.success) {
        setAllPayments(paymentsResult.data.payments || []);
      }
  ...
  return (
    <div style={{ padding: '20px' }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{
              fontSize: '24px',
              fontWeight: '700',
              color: summary.total_debt > 0 ? 'var(--color-danger)' : 'var(--color-success)'
            }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '24px', fontWeight: '700', color: 'var(--color-primary)' }}>
              ${summary.total_paid_debt.toFixed(2)}
            </div>
          </div>
        </div>
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        >
          📜 Items de Deuda ({debt_items.length})
        </button>
        <button
        ...
        >
          💰 Pagos a Deuda ({debt_payments.length})
        </button>
      </div>

      {/* Content */}
      {activeTab === 'items' && (
        <DebtItemsTable items={debt_items} />
      )}

      {activeTab === 'payments' && (
        <DebtPaymentsTable payments={debt_payments} allPayments={allPayments} />
      )}
    </div>
  );
};
